...
import { FaSubscript, FaVideo, FaUsers } from "react-icons/fa";
        ...
        <div className={`flex-1 transition-all duration-300 ${sidebarOpen ? "ml-64" : "ml-0"}`}>
              ...
              <div className="flex gap-4 items-start md:items-center">
                <div className="w-20 h-20 rounded-full bg-yt-hover flex items-center justify-center text-3xl font-bold text-yt-secondary flex-shrink-0 overflow-hidden">
                  {channel.channelAvatar ? (
                    <img src={channel.channelAvatar} alt="Channel" className="w-full h-full object-cover" />
                  ) : (
                    channel.channelName?.charAt(0)
                  )}
                </div>
                <div>
                  <h1 className="text-3xl font-bold text-yt-text">{channel.channelName}</h1>
                  <p className="text-yt-secondary mt-1">
                    {channel.subscribers || 0} subscribers • {channel.videos?.length || 0} videos
                  </p>
                  <p className="text-yt-text text-sm mt-3 line-clamp-2">{channel.description}</p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-3 gap-6 px-6 py-8 border-b border-yt-hover">
            <div className="text-center">
              <p className="text-2xl font-bold text-yt-red">{channel.videos?.length || 0}</p>
              <p className="text-yt-secondary text-sm mt-2">Videos</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-yt-red">{channel.subscribers || 0}</p>
          ...
          <div className="px-6 py-8">
            <h2 className="text-2xl font-bold text-yt-text mb-6">Latest Videos</h2>
            
            {!channel.videos || channel.videos.length === 0 ? (
              <p className="text-yt-secondary text-center py-12">No videos yet</p>
            ) : (
